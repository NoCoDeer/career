...
if (isset($featjobs_posts) && !empty($featjobs_posts)) {
    ...
    ?>
    <div class="careerfy-jobs-listing careerfy-joblisting-sten">
        <?php
        foreach ($featjobs_posts as $fjobs_post) {
            ...
            $job_type_str = function_exists('jobsearch_job_get_all_jobtypes') ? jobsearch_job_get_all_jobtypes($job_id, 'careerfy-joblisting-sten-status careerfy-part-status', '', '', '', '', 'a', 'no_color') : '';
            $sector_str = function_exists('jobsearch_job_get_all_sectors') ? jobsearch_job_get_all_sectors($job_id, '', '', '', '<span class="careerfy-joblst-optitm"><i class="careerfy-icon careerfy-filter-tool-black-shape"></i>', '</span>') : '';
            ...
            ?>
            <div class="careerfy-jobs-listing-item">

                <div class="careerfy-jobs-listing-item-inn">
                    <?php
                    ...
                    if ($post_thumbnail_src != '') {
                         ?>
                        <div class="careerfy-joblst-thumb">
                            <img class="careerfy-joblst-thumb-inn" src="<?php echo esc_url($post_thumbnail_src) ?>" alt="<?php echo get_the_title($job_id); ?>"></img>
                        </div>
                        <?php 
                    }
                    ?>
                    <div class="careerfy-joblst-desc">
                        
                        <span class="careerfy-joblst-desc-title">
                            <a href="<?php echo $quick_apply_job == 'on' && !wp_is_mobile() ? 'javascript:void(0)' : esc_url(get_permalink($job_id)); ?>"
                                ...
                                <?php
                                $job_title = esc_html(jobsearch__listing_itm_title_limit(get_the_title($job_id), $jobsearch_jobs_title_limit));
                            ...
                            if ($jobsearch_job_featured == 'on') {
                                ?>
                                <span class="careerfy-joblst-descftr"><i class="fa fa-star"></i> <?php echo esc_html__('Featured', 'careerfy'); ?></span> 
                                <?php
                            }
                            ?>                            
                        </span>

                        <div class="careerfy-joblst-opt">
                            <?php 
                            if (!empty($company_name)) {
                                ?>
                                <span class="careerfy-joblst-optitm"><?php echo($company_name) ?></span>
                                <?php
                            }
                            ...
                            if ($job_post_date != '' && $job_views_publish_date == 'on') {
                                ob_start();
                                ?>
                                <span class="careerfy-joblst-optitm">
                                ...
                                echo apply_filters('jobsearch_job_listingitm_publish_date_html', $publishdte_html, $job_id, $job_post_date, 'listing-11');
                            }

                            if (($job_salary != '') || (!empty($job_type_str) && $job_types_switch !== 'off')) { 
                                echo '<div class="careerfy-joblst-type-salary">';
                                    if ($job_salary != '') { 
                                        ?>
                                        <span class="careerfy-joblst-optitm">
                                            <i class="careerfy-icon careerfy-money"></i>
                                            <?php echo jobsearch_esc_html(str_replace('/', '', $job_salary)); ?>
                                        </span>
                                        <?php 
                                    }
                                    
                                    if (!empty($job_type_str) && $job_types_switch !== 'off') {
                                        echo '<span class="careerfy-joblst-optitm careerfy-joblst-type"><i class="careerfy-icon careerfy-clock-hours"></i> ';
                                        echo($job_type_str);
                                        echo '</span>';
                                    }
                                echo '</div>';
                ...
                <div class="careerfy-jobs-listing-lstsec">

                    <?php 

                    if ($job_city_title != '' && $all_location_allow == 'on') {
                                ?>
                                <span class="careerfy-joblst-optitm"><i class="careerfy-icon careerfy-map-pin"></i> <?php echo($job_city_title);?></span>
                                <?php
                            }

                    if($quick_apply_job == 'on'){
                        ?>
                        <div class="careerfy-joblst-moreopt-btnswrp">
                            <a href="javascript:void(0);" data-job-id="<?php echo esc_html($job_id); ?>" class="<?php echo($quick_apply_job_btn) ?> careerfy-joblst-moreopt-btns careerfy-joblst-moreopt-btnsapply"><i class="careerfy-icon careerfy-briefcase-work"></i> <?php esc_html__('Apply Job', 'careerfy');?></a>
                        </div>
                        <?php
                    }
                    ?>
                    ...
                    $book_mark_args = array(
                        ...
                        'after_label' => esc_html__('', 'careerfy'),
                        'before_label' => esc_html__('', 'careerfy'),
                        'container_class' => '',
                        'anchor_class' => 'careerfy-jobs-like-style9 careerfy-joblst-like',
                        'view' => 'style9',
                    );
...
?>
<div class="careerfy-jobs-listing careerfy-joblisting-sten" id="jobsearch-job-<?php echo absint($job_short_counter) ?>">    
    <?php
